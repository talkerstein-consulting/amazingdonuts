...
const slotsFor=(dateValue:string)=>{const date=new Date(`${dateValue}T12:00:00`),window=HOURS[date.getDay()];if(!window)return [];return Array.from({length:(window[1]-window[0])/15+1},(_,index)=>{const minutes=window[0]+index*15,hour=Math.floor(minutes/60),minute=minutes%60,value=`${String(hour).padStart(2,'0')}:${String(minute).padStart(2,'0')}`,label=new Intl.DateTimeFormat('en-CA',{hour:'numeric',minute:'2-digit'}).format(new Date(2000,0,1,hour,minute));return {value,label}})};
...
function Checkout(){
  ...
  const fulfillmentTimes=slotsFor(scheduledAt.slice(0,10));
  ...
  return <main className="commerce-shell">
    ...
    <div className="checkout-grid"><form className="checkout-form" onSubmit={place}>
      ...
      <fieldset disabled={!session?.user||busy}><legend>Fulfillment</legend>
        ...
        <div className="fulfillment-schedule"><label><span>Date</span><input type="date" min={scheduledMinimum.slice(0,10)} value={scheduledAt.slice(0,10)} onChange={event=>setScheduledAt(`${event.target.value}T09:00`)} required/></label><label><span>Time</span><select value={fulfillmentTimes.length?scheduledAt.slice(11,16):''} disabled={!fulfillmentTimes.length} onChange={event=>setScheduledAt(`${scheduledAt.slice(0,10)}T${event.target.value}`)} required>{fulfillmentTimes.length?fulfillmentTimes.map(slot=><option value={slot.value} key={slot.value}>{slot.label}</option>):<option value="">Closed</option>}</select></label>{requiresPrintLeadTime&&<small>Custom-printed items require at least one week's notice.</small>}<small className="schedule-hours">Sunday 8am–1pm · Monday–Thursday 7:30am–4pm · Friday 7:30am–2pm · Saturday closed. Times are in 15-minute intervals.</small></div>
      ...
      <fieldset disabled={!session?.user||busy}><legend>Payment</legend><div className="payment-options"><button type="button" className={method==='card'?'active':''} onClick={()=>setMethod('card')}><CreditCard/><span><strong>Card</strong><small>Securely processed by Square</small></span></button>{session?.houseAccount?.status==='active'&&session.houseAccount.creditEnabled&&<button type="button" className={method==='house_account'?'active':''} onClick={()=>setMethod('house_account')}><Building2/><span><strong>Pay on account</strong><small>{session.houseAccount.organizationName} · {money(session.houseAccount.credit.available/100)} credit available</small></span></button>}</div>{method==='card'&&<div id="square-card" className="square-card"/>}{method==='card'&&session?.houseAccount?.status==='active'&&!session.houseAccount.creditEnabled&&<p className="house-note">After entering your card, choose whether to pay now without saving it or securely save it with Square and use account credit.</p>}{method==='house_account'&&<p className="house-note">This order will be added to your account balance and included on your next statement. Your saved card may be charged when the statement is due.</p>}</fieldset>
      {(quoteError||error)&&<p className="checkout-error" role="alert">{quoteError||error}</p>}
      <button className="place-order" disabled={!session?.user||!lines.length||!customReady||!quote||busy}>{busy?'Uploading artwork and placing order...':!customReady?'Finish custom items in your cart':quote?`Place order · ${money(payable)}`:'Calculating Square total...'}</button>
    ...
    {confirmPayment&&<div className="payment-confirm-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setConfirmPayment(false)}}><section className="payment-confirm" role="dialog" aria-modal="true" aria-labelledby="payment-confirm-title"><CreditCard/><p>Confirm payment</p><h2 id="payment-confirm-title">How would you like to pay?</h2><span>{session?.houseAccount?.creditEnabled?'Your card can be charged now, or this order can use your approved account credit and be invoiced through Square.':'Pay this order now without saving your card, or authorize Square to save it and use your approved account credit.'}</span><div><button type="button" onClick={()=>void place(undefined,true)}>Pay this order now · {money(payable)}</button>{session?.houseAccount?.creditEnabled?<button type="button" onClick={()=>{setMethod('house_account');setConfirmPayment(false)}}>Use account credit</button>:<button type="button" onClick={()=>void saveAndUseCredit()}>Save card and use account credit</button>}</div><button className="payment-confirm-cancel" type="button" onClick={()=>setConfirmPayment(false)}>Return to checkout</button></section></div>}
  </main>;
}
