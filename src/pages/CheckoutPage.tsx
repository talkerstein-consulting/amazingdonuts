...
function Checkout(){
  ...
  return <main className="commerce-shell">
    ...
    {confirmPayment&&<div className="payment-confirm-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)setConfirmPayment(false)}}><section className="payment-confirm" role="dialog" aria-modal="true" aria-labelledby="payment-confirm-title"><CreditCard/><p>Confirm payment</p><h2 id="payment-confirm-title">How would you like to pay?</h2><span>Your card can be charged now, or this order can use your approved account credit and be invoiced through Square.</span><div><button type="button" onClick={()=>void place(undefined,true)}>Charge card now · {money(payable)}</button><button type="button" onClick={()=>{setMethod('house_account');setConfirmPayment(false)}}>Use account credit</button></div><button className="payment-confirm-cancel" type="button" onClick={()=>setConfirmPayment(false)}>Return to checkout</button></section></div>}
  </main>;
}
