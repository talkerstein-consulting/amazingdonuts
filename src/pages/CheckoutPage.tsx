...
function Checkout(){
  ...
  return <main className="commerce-shell">
    <header className="commerce-top"><a href="/shop/"><ArrowLeft/> <span>Back to the shop</span></a><CommerceLogo/><a href="/account/">My account</a></header>
    <div className="checkout-grid"><form id="checkout" className="checkout-form" onSubmit={place}>
      <div className="commerce-heading"><p>Secure checkout</p><h1>Finish your order</h1><span><LockKeyhole/> <span>Sign-in is required. Guest checkout is not available.</span></span></div>
      ...
      <fieldset className="payment-panel" form="checkout" disabled={!session?.user||busy}><legend>Payment</legend>
        ...
        <div className="payment-options">
          {session?.user&&quote?.order?.total&&wallets.apple!=='unavailable'&&<div className="apple-pay-slot">{wallets.apple==='ready'?<button type="button" className="apple-pay-button" aria-label="Pay with Apple Pay" onClick={()=>payWithWallet(applePay.current)}/>:<div className="wallet-loading" aria-hidden="true"/>}</div>}
          {session?.user&&quote?.order?.total&&wallets.google!=='unavailable'&&<div className="google-pay-slot"><div id="google-pay-button" className={wallets.google==='ready'?'is-ready':''} onClick={()=>payWithWallet(googlePay.current)}/>{wallets.google==='loading'&&<div className="wallet-loading" aria-hidden="true"/>}</div>}
...
<p>Your bag</p><h2>{lines.reduce((total,line)=>total+line.qty,0)} items</h2>{lines.map(line=><div className="summary-line" key={line.product.id}><img src={line.product.img} alt=""/><span><strong>{line.product.name}</strong><small>Qty {line.qty}</small></span><b>{money(Number(line.product.price.replace(/[^0-9.]/g,''))*line.qty)}</b></div>)}<div className="summary-breakdown"><span>Merchandise</span><b>{quote?money((quote.order.subtotal-(quote.delivery?.fee||0))/100):money(subtotal)}</b>{quote?.delivery&&<><span>Delivery</span><b>{quote.delivery.free?'Free':money(quote.delivery.fee/100)}</b></>}{quote&&<><span>HST</span><b>{money(quote.order.tax/100)}</b></>}</div><div className="summary-total"><span>{quote?'Total':'Estimated subtotal'}</span><strong>{money(quote?payable:subtotal)}</strong></div><small>{quote?.delivery?'The bakery assigns the driver after payment. Square does not dispatch the courier.':'Square confirms catalog pricing, HST, discounts, and the final total before payment.'}</small></aside>
      ...
      <button className="place-order" form="checkout" disabled={!session?.user||!lines.length||!customReady||!quote||!method||busy}>{busy?'Uploading artwork and placing order...':!customReady?'Finish custom items in your cart':!method?'Choose how to pay':quote?`Place order · ${money(payable)}`:'Calculating Square total...'}</button>
