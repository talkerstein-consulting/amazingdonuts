...
export default function CollectionRail({
  ...
  useEffect(() => {
    ...
    const selected = element?.querySelector<HTMLElement>('.is-on');
    if (!element || !selected || !compact) return;
    const reveal = () => {
      ...
      if (bounds.left < viewport.left + gutter) {
        element.scrollBy({ left: bounds.left - viewport.left - gutter, behavior });
      } else if (bounds.right > viewport.right - gutter) {
        element.scrollBy({ left: bounds.right - viewport.right + gutter, behavior });
      }
    };
    reveal();
    element.addEventListener('transitionend', reveal);
    return () => element.removeEventListener('transitionend', reveal);
  }, [active, compact]);
  ...
  const arrows = (inline: boolean) => (
    <div
      className={`collections__arrows${inline ? ' collections__arrows--inline' : ''}`}
      aria-hidden={inline !== compact}
    >
      <button
        type="button"
        onClick={() => nudge(-1)}
        disabled={!edges.start || inline !== compact}
        tabIndex={inline !== compact ? -1 : undefined}
      ...
      <button
        type="button"
        onClick={() => nudge(1)}
        disabled={!edges.end || inline !== compact}
        tabIndex={inline !== compact ? -1 : undefined}
  ...
  return (
    ...
    >
      {/* Keep the heading mounted so its height can animate in both directions. */}
      <div className="collections__heading" aria-hidden={compact}>
        ...
        {cards.map((card) => {
          ...
          return (
            <button
              ...
              aria-pressed={on}
              aria-current={on && !showAll ? 'location' : undefined}
            ...
            >
              ...
              <span className="collection-card__text">
                ...
                <span className="collection-card__count" aria-hidden={compact}>{card.count}</span>
              </span>
            </button>
          );
        })}
