 ...
 *   - delivery. Confirm the choice and leave scheduling to checkout.
 *
...
function fulfillmentLine(
  ...
  if (choice === 'delivery') {
    return 'You selected local delivery. Choose your delivery window at checkout.';
  }
  if (choice === 'pickup') {
          ...
          >
            ...
            <div className="return-prompt__bed" ref={cardRef}>
              {hasBag ? (
                ...
                <span className={`return-prompt__stack${shown.length === 1 ? ' is-single' : ''}`}>
                  {shown.map((line, i) => (
                    <img
                      key={line.product.id}
