...
import { authReturnTo } from '../lib/auth-navigation';
...
export default function AuthModal({ open, onClose, onSuccess }: { open: boolean; onClose: () => void; onSuccess?: () => void }) {
  ...
  const [googleAuthorizeOrigin, setGoogleAuthorizeOrigin] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!open) return;
    fetch('/api/house/auth/google/config').then(response => response.ok ? response.json() : null).then(body => { setGoogleEnabled(Boolean(body?.enabled)); setGoogleAuthorizeOrigin(body?.authorizeOrigin || null); }).catch(() => setGoogleEnabled(false));
  }, [open]);
            ...
            <div className="auth__form">
              ...
              {googleEnabled && mode !== 'forgot' && <>
                <div className="auth__providers">
                  <button type="button" className="auth__provider auth__provider--google" onClick={() => { const returnTo = authReturnTo(window.location); const url = new URL('/api/house/auth/google/start', googleAuthorizeOrigin || window.location.origin); url.searchParams.set('returnTo', returnTo); if (googleAuthorizeOrigin) url.searchParams.set('previewOrigin', window.location.origin); window.location.assign(url.toString()); }}>
              ...
              <form
                onSubmit={async (e) => {
                  ...
                  try {
                    ...
                    const destination = authReturnTo(window.location);
                    if (destination === '/account/' && window.location.pathname !== '/account/') {
                      onClose();
