...
export default function BoxBuilder({ product }: { product: Product }) {
  ...
  /* What can go in: single donuts at counter prices. The custom-printed dozen
     ...
     Read from `SHOP_PRODUCTS`, which is the catalogue minus the internal rows,
     so the Donut Lab's own line cannot appear here — it is a Donut at under $5
     and used to show up as a flavour you could drop into a box, which is an
  ...
  const packable = counts.includes(chosen.length);
  ...
  const shuffleButton = (
    /* Underlined text, not a second knob. It is the way out of choosing rather
       ...
       work. */
    <button type="button" className="boxer__shuffle" onClick={pickForMe}>
      <Shuffle size={15} strokeWidth={2.6} aria-hidden="true" />
      Pick for me
    </button>
  );

  /* The product's own price — each tray is its own catalogue row now, so the
     box costs what it costs however far through filling it you are.

     NOTE: the dozen's $24.00 is twice the half dozen, which is a placeholder.
     If the bakery gives a dozen any kind of break, it is the price on the
     `dozen-box` row in `products.ts` that changes, not a rule here. */
      ...
      <div className="boxer__inner">
        ...
        <div className="boxer__picker">
          ...
          <div className="boxer__pickerHead">
            <div>
              ...
              <p className="boxer__progress" aria-live="polite">
                ...
                {!packable && ` · ${max - chosen.length} to go`}
              </p>
            </div>
          ...
          <div className="boxer__foot">
            ...
            >
              {packable ? 'Add to bag' : `Choose ${max - chosen.length} more`}
            </BrandButton>
