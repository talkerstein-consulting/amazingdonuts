...
const T_HAND = 0.72; // wordmark + field move into the navbar
const T_END = T_ROLL + T_HOLD + T_HAND;
...
export default function Preloader({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      /* ---- 4. hand-off: wordmark flies to the navbar, field shrinks ----- */
      const handP = easeInOutCubic(ramp(t, T_ROLL + T_HOLD, T_ROLL + T_HOLD + T_HAND));

      if (logoRef.current) {
        const { s, ox, oy } = view;
        // Where the wordmark is now, in screen px.
        const fromW = LOGO_W * SCALE * s;
        const fromX = ox + LOGO_L * s;
        const fromY = oy + LOGO_T * s;

        const target = navLogoRect();
        if (target && handP > 0) {
          const k = target.width / fromW; // scale needed to match the navbar logo
          logoRef.current.style.transform =
            `translate(${lerp(fromX, target.left, handP).toFixed(1)}px, ${lerp(fromY, target.top, handP).toFixed(1)}px)` +
            ` scale(${lerp(1, k, handP).toFixed(4)})`;
        } else {
          logoRef.current.style.transform = `translate(${fromX.toFixed(1)}px, ${fromY.toFixed(1)}px) scale(1)`;
        }
        logoRef.current.style.width = `${fromW.toFixed(1)}px`;
        logoRef.current.style.height = `${(LOGO_H * SCALE * s).toFixed(1)}px`;
      }

      if (fieldRef.current) {
        // The navy field collapses upward until it is exactly the navbar.
        const barH = navBarHeight();
        fieldRef.current.style.height = `${lerp(window.innerHeight, barH, handP).toFixed(1)}px`;
      }

      if (stageWrapRef.current) {
        // Crumbs and the roll fade out as the hand-off begins.
        stageWrapRef.current.style.opacity = (1 - ramp(t, T_ROLL + T_HOLD, T_ROLL + T_HOLD + 0.3)).toFixed(3);
      }
  ...
  return (
    <div className="pre" role="status" aria-label="Loading Amazing Donuts">
      ...
      {/* crumbs + the rolling donut, in authored stage coordinates */}
      ...
      {/* the wordmark — inked, then flown into the navbar */}
