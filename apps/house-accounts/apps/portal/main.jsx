...
function AccountList({ accounts, account, query, setQuery, setSelected, canCreate, demo, onCreated }) {
  const blankAddress={addressLine1:"",addressLine2:"",locality:"Toronto",administrativeDistrictLevel1:"ON",postalCode:"",country:"CA"};
  const [creating,setCreating]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState(""),[address,setAddress]=useState(blankAddress);
    ...
    <div className="account-list">
      <div className="section-head">
        <div>
          ...
          {canCreate ? <button className="account-create-trigger" type="button" onClick={()=>{setAddress(blankAddress);setCreating(true);}}><Plus/> Add account</button> : null}
        </div>
      ...
      {creating ? <div className="account-create-backdrop" role="presentation" onMouseDown={(event)=>{if(event.target===event.currentTarget)setCreating(false);}}><section className="account-create-modal" role="dialog" aria-modal="true" aria-labelledby="create-account-title"><header><div><span>Institutional accounts</span><h2 id="create-account-title">Add an organization</h2></div><button type="button" onClick={()=>setCreating(false)} aria-label="Close">×</button></header><form onSubmit={createAccount}><label><span>Organization name</span><input name="organizationName" required/></label><label><span>Organization type</span><select name="organizationType">{Object.keys(organizationRoles).map(type=><option key={type}>{type}</option>)}</select></label><label><span>Billing contact</span><input name="billingContact" required/></label><label><span>Billing email</span><input name="billingEmail" type="email" required/></label><label><span>Phone</span><input name="phone" type="tel" onInput={event=>event.currentTarget.value=formatPhone(event.currentTarget.value)}/></label><label><span>Credit limit</span><div className="money-input"><span aria-hidden="true">$</span><input name="creditLimit" type="number" min="0" step=".01" required/></div></label><label><span>Terms</span><select name="paymentTermsDays" defaultValue="30"><option value="0">Due upon receipt</option><option value="15">Net 15</option><option value="30">Net 30</option><option value="45">Net 45</option><option value="60">Net 60</option></select></label><label><span>Invoice schedule</span><select name="billingFrequency" defaultValue="manual"><option value="manual">Manual</option><option value="weekly">Weekly</option><option value="monthly">Monthly</option></select></label><label><span>Organization PIN</span><input name="organizationPin" type="password" inputMode="numeric" pattern="[0-9]{4,8}" minLength="4" maxLength="8" required/></label><label className="wide"><span>Street address</span><AdminAddressAutocomplete address={address} onChange={setAddress}/><input type="hidden" name="addressLine1" value={address.addressLine1}/></label><label><span>Unit</span><input name="addressLine2" value={address.addressLine2} onChange={event=>setAddress({...address,addressLine2:event.target.value})}/></label><label><span>City</span><input name="locality" value={address.locality} onChange={event=>setAddress({...address,locality:event.target.value})} required/></label><label><span>Province</span><input name="province" value={address.administrativeDistrictLevel1} onChange={event=>setAddress({...address,administrativeDistrictLevel1:event.target.value.toUpperCase()})} required/></label><label><span>Postal code</span><input name="postalCode" value={address.postalCode} onChange={event=>setAddress({...address,postalCode:event.target.value.toUpperCase()})} required/></label>{error?<p className="error wide">{error}</p>:null}<footer className="wide"><button type="button" className="secondary-button" onClick={()=>setCreating(false)}>Cancel</button><button disabled={busy}>{busy?"Creating...":"Create account"}</button></footer></form></section></div> : null}
    </div>
  );
}
function AdminAddressAutocomplete({address,onChange}){
  const [suggestions,setSuggestions]=useState([]),[open,setOpen]=useState(false),[busy,setBusy]=useState(false),[error,setError]=useState("");
  const token=useRef(crypto.randomUUID()),timer=useRef();
  useEffect(()=>()=>clearTimeout(timer.current),[]);
  const search=value=>{onChange({...address,addressLine1:value});setError("");clearTimeout(timer.current);if(value.trim().length<3){setSuggestions([]);setOpen(false);return;}timer.current=setTimeout(async()=>{try{const body=await request("/admin/address-search",{method:"POST",body:JSON.stringify({input:value,sessionToken:token.current})});setSuggestions(body.suggestions||[]);setOpen(true);}catch(cause){setError(cause.message);}},250);};
  const choose=async suggestion=>{setBusy(true);setError("");try{const body=await request(`/admin/address-search/${encodeURIComponent(suggestion.placeId)}?sessionToken=${encodeURIComponent(token.current)}`);onChange({...address,...body.address,addressLine2:address.addressLine2||body.address.addressLine2||""});setSuggestions([]);setOpen(false);token.current=crypto.randomUUID();}catch(cause){setError(cause.message);}finally{setBusy(false);}};
  return <div className="admin-address-autocomplete"><input value={address.addressLine1} onChange={event=>search(event.target.value)} onFocus={()=>suggestions.length&&setOpen(true)} onBlur={()=>setTimeout(()=>setOpen(false),150)} autoComplete="street-address" placeholder="Start typing an address" required/>{busy?<small className="admin-address-status">Finding address...</small>:null}{open&&suggestions.length?<div className="admin-address-suggestions" role="listbox">{suggestions.map(suggestion=><button type="button" role="option" key={suggestion.placeId} onMouseDown={event=>event.preventDefault()} onClick={()=>choose(suggestion)}><span><strong>{suggestion.mainText}</strong><small>{suggestion.secondaryText}</small></span></button>)}<footer>Address suggestions by Google</footer></div>:null}{error?<small className="admin-address-error">{error}</small>:null}</div>;
}
