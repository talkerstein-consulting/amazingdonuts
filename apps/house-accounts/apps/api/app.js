...
export function createApp({ pool, square, uberDirect, config }) {
  ...
  app.delete("/api/storefront/account",async(request,response,next)=>{try{
    ...
    await transaction(pool,async client=>{await client.query("DELETE FROM sessions WHERE user_id=$1",[user.id]);await client.query("DELETE FROM password_reset_tokens WHERE user_id=$1",[user.id]);await client.query("DELETE FROM user_identities WHERE user_id=$1",[user.id]);await client.query("DELETE FROM user_wishlist WHERE user_id=$1",[user.id]);await client.query("DELETE FROM customer_addresses WHERE user_id=$1",[user.id]);await client.query("DELETE FROM customer_profiles WHERE tenant_id=$1 AND user_id=$2",[user.tenant_id,user.id]);await client.query("DELETE FROM account_users WHERE user_id=$1",[user.id]);await client.query("UPDATE tenant_memberships SET status='disabled' WHERE tenant_id=$1 AND user_id=$2",[user.tenant_id,user.id]);await client.query("UPDATE users SET email=$2,password_hash=NULL,first_name='Deleted',last_name='Customer',phone=NULL,status='disabled' WHERE id=$1",[user.id,`deleted-${user.id}@deleted.invalid`]);});
