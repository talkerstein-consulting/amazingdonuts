...
async function buildSquareOrder(square,locationId,input,user){
  ...
  const lineItems=input.items.map(line=>{
    ...
    const custom=input.customizations?.find(entry=>normalizeName(entry.productName)===wanted),note=custom?.kind==="glyph"?`CUSTOM GLYPH: ${custom.glyph}`:custom?.kind==="print"?`CUSTOM PRINT: ${custom.icingFlavor} icing${custom.sprinkleColours?`, ${custom.sprinkleColours} sprinkles`:""}. ${custom.artworks.map((art,index)=>`Design ${index+1} x ${art.count}`).join(", ")}. Artwork files are in the Amazing Donuts order portal.`:undefined;
