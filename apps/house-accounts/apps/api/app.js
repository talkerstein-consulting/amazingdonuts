...
export function createApp({ pool, square, config }) {
  ...
  app.get("/api/storefront/config",(_request,response)=>response.json({environment:config.squareEnvironment,applicationId:config.squareApplicationId,locationId:config.squareLocationId,currency:"CAD"}));
  app.post("/api/storefront/quote",async(request,response,next)=>{try{const user=requireUser(request),input=z.object({items:cartSchema,fulfillment:fulfillmentSchema}).parse(request.body),tenantSquare=typeof square.forTenant==="function"?await square.forTenant(user.tenant_id):square,order=await buildSquareOrder(tenantSquare,config.squareLocationId,input,user);const calculated=await tenantSquare.request("/v2/orders/calculate",{method:"POST",body:{order}});response.json({order:publicOrder(calculated.order)});}catch(error){next(error);}});
  app.post("/api/storefront/checkout",async(request,response,next)=>{try{
    ...
    const orderDraft=await buildSquareOrder(tenantSquare,config.squareLocationId,input,user),calculated=(await tenantSquare.request("/v2/orders/calculate",{method:"POST",body:{order:orderDraft}})).order;
    let account=null;if(input.paymentMethod==="house_account"){const result=await pool.query("SELECT a.* FROM account_users au JOIN accounts a ON a.id=au.account_id WHERE au.user_id=$1 AND a.tenant_id=$2 AND au.status='active' AND a.status='active' LIMIT 1",[user.id,user.tenant_id]);if(!result.rowCount)throw Object.assign(new Error("An active house account is required."),{status:403});account=result.rows[0];calculated.customer_id=account.square_customer_id;await reserveCredit(pool,{accountId:account.id,amount:Number(calculated.total_money.amount),idempotencyKey:input.idempotencyKey});}
    const created=(await tenantSquare.createOrder({idempotency_key:`order-${input.idempotencyKey}`,order:{...orderDraft,...(account?{customer_id:account.square_customer_id}:{})}})).order;
    const payment=(await tenantSquare.createPayment({idempotency_key:`payment-${input.idempotencyKey}`,source_id:input.paymentMethod==="house_account"?"EXTERNAL":input.sourceId,amount_money:created.total_money,order_id:created.id,location_id:config.squareLocationId,customer_id:account?.square_customer_id||undefined,autocomplete:true,...(input.paymentMethod==="house_account"?{external_details:{type:"OTHER",source:"Online House Account"}}:{})})).payment;
    ...
    });response.status(201).json({order:publicOrder(created),paymentMethod:input.paymentMethod});
  }catch(error){next(error);}});
...
async function buildSquareOrder(square,locationId,input,user){
  ...
  const fulfillment=input.fulfillment.type==="pickup"?{type:"PICKUP",state:"PROPOSED",pickup_details:{schedule_type:"SCHEDULED",pickup_at:input.fulfillment.scheduledAt,recipient}}:{type:"DELIVERY",state:"PROPOSED",delivery_details:{schedule_type:"SCHEDULED",deliver_at:input.fulfillment.scheduledAt,recipient}};
  return {location_id:locationId,line_items:lineItems,fulfillments:[fulfillment],taxes:taxes.map((tax,index)=>({uid:`catalog-tax-${index}`,catalog_object_id:tax.id})),pricing_options:{auto_apply_taxes:true,auto_apply_discounts:true},source:{name:"Amazing Donuts Website"},reference_id:`web-${user.id.slice(0,8)}-${Date.now()}`};
}
