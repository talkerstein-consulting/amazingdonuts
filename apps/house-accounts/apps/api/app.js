...
const customizationsSchema=z.array(z.discriminatedUnion("kind",[z.object({productName:z.string().min(1).max(180),kind:z.literal("print"),artworks:z.array(artworkSchema).min(1).max(99)}),z.object({productName:z.string().min(1).max(180),kind:z.literal("glyph"),glyph:z.string().regex(/^[A-Z0-9]$/)})])).max(20).default([]);
...
async function buildSquareOrder(square,locationId,input,user){
  ...
  const lineItems=input.items.map(line=>{
    ...
    const custom=input.customizations?.find(entry=>normalizeName(entry.productName)===wanted),note=custom?.kind==="glyph"?`CUSTOM GLYPH: ${custom.glyph}`:custom?.kind==="print"?`CUSTOM PRINT: ${custom.artworks.map((art,index)=>`Design ${index+1} x ${art.count}`).join(", ")}. Artwork files are in the Amazing Donuts order portal.`:undefined;
