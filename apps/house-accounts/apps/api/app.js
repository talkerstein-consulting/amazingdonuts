...
export function createApp({ pool, square, config }) {
  ...
  app.get("/api/jobs/statements",async(request,response,next)=>{try{
    ...
    const collections=await collectDueStatements(pool,square,config);
    ...
    response.json({ok:true,issued,collections,queued:pending.rowCount,sent,emailConfigured});
  }catch(error){next(error);}});
