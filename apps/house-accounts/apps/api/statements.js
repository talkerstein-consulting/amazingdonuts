...
export async function statementPdf(statement, tenant, account) {
  ...
  addPage();
  page.drawText(draft?"Current statement preview":"Account details",{x:42,y,size:18,font:bold,color:ink}); y-=28;
  const lastPayment=payments[0]?.received_at||statement.paid_at||statement.snapshot.lastPayment;
  const columns = [["Bill to",`${account.billing_contact}\n${account.organization_name}`],[draft?"As of":"Invoice period",`${statementDate(statement.period_start)} - ${statementDate(statement.period_end)}`],["Last payment",lastPayment ? statementDate(lastPayment) : "N/A"],["Payment status",draft?"Not yet issued":balanceDue?`${cash(balanceDue,statement.currency)}\nDue ${statementDate(statement.due_at)}`:`Paid in full\n${lastPayment?statementDate(lastPayment):""}`]];
  ...
  y-=84; page.drawText(draft?"Current balance summary":"Account statement summary",{x:42,y,size:18,font:bold,color:ink}); y-=32;
  ...
  page.drawLine({start:{x:42,y:y+10},end:{x:570,y:y+10},thickness:1,color:rule}); row(draft?"Current balance (not yet due)":balanceDue?`Total payment due (${statementDate(statement.due_at)})`:"Balance due",cash(balanceDue,statement.currency)); y-=44;
  if(draft){page.drawText("This is a live preview. The final bill and due date are set when the statement is issued.",{x:42,y,size:8,font:regular,color:ink});y-=28;}
  if(payments.length){
