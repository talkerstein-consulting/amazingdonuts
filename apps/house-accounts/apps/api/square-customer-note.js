...
export async function syncInstitutionalPinByEmail(square,email,pin,additionalCustomerIds=[]){
  if(!email||!pin)return [];
  const found=await square.searchCustomers({query:{filter:{email_address:{exact:String(email).toLowerCase()}}},limit:100}),customerIds=[...new Set([...additionalCustomerIds,...(found.customers||[]).map(customer=>customer.id)].filter(Boolean))];
  await Promise.all(customerIds.map(customerId=>syncInstitutionalPinNote(square,customerId,pin)));
  return customerIds;
}
