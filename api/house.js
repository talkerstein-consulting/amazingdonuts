...
function getApp() {
  ...
  const uberDirectMode=process.env.UBER_DIRECT_MODE||"sandbox";
  const uberDirect=new UberDirectClient({clientId:process.env.UBER_DIRECT_CLIENT_ID||"",clientSecret:process.env.UBER_DIRECT_CLIENT_SECRET||"",customerId:process.env.UBER_DIRECT_CUSTOMER_ID||"",mode:uberDirectMode});
  app=createApp({pool,square,uberDirect,config:{secureCookies:true,sessionSecret:process.env.SESSION_SECRET||"",googleClientId:process.env.GOOGLE_OAUTH_CLIENT_ID||"",googleClientSecret:process.env.GOOGLE_OAUTH_CLIENT_SECRET||"",googlePlacesApiKey:process.env.GOOGLE_PLACES_API_KEY||"",squareEnvironment:process.env.SQUARE_ENVIRONMENT||"sandbox",squareApplicationId:process.env.SQUARE_APPLICATION_ID||"",squareLocationId:process.env.SQUARE_LOCATION_ID||"",squareWebhookSignatureKey:process.env.SQUARE_WEBHOOK_SIGNATURE_KEY||"",squareWebhookNotificationUrl:process.env.SQUARE_WEBHOOK_NOTIFICATION_URL||"",institutionalTenderNames:(process.env.SQUARE_INSTITUTIONAL_TENDER_NAMES||"Institutional Account Credit").split(",").map(value=>value.trim()).filter(Boolean),cronSecret:process.env.CRON_SECRET||"",smtpHost:process.env.SMTP_HOST||"",smtpPort:Number(process.env.SMTP_PORT||465),smtpSecure:process.env.SMTP_SECURE!=="false",smtpUser:process.env.SMTP_USER||"",smtpPassword:process.env.SMTP_PASSWORD||"",emailFrom:process.env.ACCOUNTS_EMAIL_FROM||"Amazing Donuts <accounts@amazingdonuts.com>",emailReplyTo:process.env.ACCOUNTS_REPLY_TO||process.env.SMTP_USER||"",ownerEmails:(process.env.OWNER_NOTIFICATION_EMAILS||process.env.ADMIN_EMAILS||"").split(",").map(value=>value.trim()).filter(Boolean),siteUrl:process.env.PUBLIC_SITE_URL||"https://amazing-donuts.vercel.app",delivery:deliveryConfig(),deploymentMode:process.env.DEPLOYMENT_MODE||"production",mixedEnvironmentTestMode:process.env.MIXED_ENVIRONMENT_TEST_MODE==="true",uberDirectMode,uberDirectAutoDispatch:process.env.UBER_DIRECT_AUTO_DISPATCH==="true",uberDirectWebhookSigningKey:process.env.UBER_DIRECT_WEBHOOK_SIGNING_KEY||"",uberPickup:{name:process.env.UBER_PICKUP_NAME||"Amazing Donuts",phone:process.env.UBER_PICKUP_PHONE||"",addressLine1:process.env.UBER_PICKUP_ADDRESS_LINE_1||"3499 Bathurst Street",addressLine2:process.env.UBER_PICKUP_ADDRESS_LINE_2||"",locality:process.env.UBER_PICKUP_CITY||"Toronto",province:process.env.UBER_PICKUP_PROVINCE||"ON",postalCode:process.env.UBER_PICKUP_POSTAL_CODE||"",country:process.env.UBER_PICKUP_COUNTRY||"CA",instructions:process.env.UBER_PICKUP_INSTRUCTIONS||"Pick up at the front counter."}}});
  return app;
}
