...
if (process.env.SQUARE_ENVIRONMENT !== "production" || !process.env.SQUARE_ACCESS_TOKEN || !process.env.SQUARE_LOCATION_ID) {
  throw new Error("Explicit production Square credentials and location are required.");
}
...
if (!template?.present_at_location_ids?.includes(locationId)) throw new Error("Expected donut template/location not found.");
const categories = template.item_data.categories.map(({ id }) => ({ id }));
if (!categories.some(({ id }) => active.find(object => object.id === id)?.category_data?.category_type === "MENU_CATEGORY")) {
...
for (const box of DONUT_BOXES) {
  ...
  objects.push({ type: "ITEM", id: itemId, present_at_all_locations: false, present_at_location_ids: [locationId],
    item_data: { name: box.name, description: `${box.title}. Choose ${box.size} donuts from the available box flavours.`,
      ...
      categories, reporting_category: { id: template.item_data.reporting_category.id }, channels: template.item_data.channels,
      ...
      variations: [{ type: "ITEM_VARIATION", id: `#${box.sku}-regular`, present_at_all_locations: false, present_at_location_ids: [locationId],
        item_variation_data: { item_id: itemId, name: `${box.size} donuts`, sku: box.sku, pricing_type: "FIXED_PRICING",
          price_money: { amount: box.amount, currency: "CAD" }, sellable: true, stockable: true, track_inventory: false, channels: template.item_data.channels } }] } });
}
